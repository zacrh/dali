            ...
            <div className="flex flex-col w-full mt-2">
                <div className="flex flex-row gap-2 items-center mb-4 px-3">
                    ...
                    <div className="flex flex-col">
                        <h2 className="text-md font-semibold truncate max-w-[95px]">{session?.user?.member?.name.split(" ")[0]}</h2>
                        <p className="text-sm text-gray-600 dark:text-slate-500 truncate max-w-[95px]">{session?.user?.member?.name.split(" ")[1]}</p>
                    </div>
                </div>

                <Link href="/home" className="flex flex-row items-center gap-2 hover:bg-gray-50 dark:hover:bg-secondary px-3 py-2 rounded-lg">
                {
                    pathName === "/home" ? (
                    ...
                    <p className={`text-lg ${pathName === "/home" ? "font-bold" : "font-medium"}`}>Home</p>
                </Link>
                ...
                <Link href={`/projects`} className="flex flex-row items-center gap-2 hover:bg-gray-50 dark:hover:bg-secondary px-3 py-2 rounded-lg">
                    ...
                    <p className={`text-lg font-medium ${pathName === `/projects` && "font-bold"}`}>Projects</p>
                </Link>
                ...
                <Link href={`/profile/${session?.user?.member?.id}`} className="flex flex-row items-center gap-2 hover:bg-gray-50 dark:hover:bg-secondary px-3 py-2 rounded-lg">
                    ...
                    <p className={`text-lg font-medium ${pathName === `/profile/${session?.user?.member?.id}` && "font-bold"}`}>Profile</p>
                </Link>
