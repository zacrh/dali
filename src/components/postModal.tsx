          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <div className="flex flex-row justify-between items-center px-2 py-2">
                <button
                  className="px-2 py-1 text-primary rounded-full hover:bg-primaryhover text-sm font-medium"
                  onClick={() => {
                    setShowModal(false);
                  }}
                >
                ...
                <button
                  className={`px-3 py-1 text-white rounded-full bg-primary hover:brightness-125 text-sm font-medium ${
                    (textContent.length === 0 ||
                      textContent.length > MAX_CHARACTERS ||
                      loading) &&
                    "opacity-80 cursor-not-allowed hover:brightness-100"
                  }`}
                  onClick={() => {
                    onPostClick();
                  }}
              ...
              <div className="flex flex-row gap-4 p-3 pt-0">
                <div className="w-1/12">
                  {session?.user?.image ? (
                    <Image
                      ...
                      alt="Profile Picture"
                      className="w-11 h-11 rounded-full border-gray-300 dark:border-border"
                        width={44}
                        height={44}
                    />
                  ) : (
                ...
                <textarea
                  ...
                  className="w-11/12 h-32 text-lg resize-none bg-white dark:bg-background border-0 font-medium outline-none border-none outline-none focus:outline-none focus:border-none focus:ring-0"
                  value={textContent}
              ...
              <div className="flex flex-row justify-between items-center px-2 py-1 border-t border-gray-300 dark:border-border">
                <div className="relative">
                  <div
                    id="project-dropdown"
                    className={`z-10 ${
                      dropdownOpen["project"] ? "absolute" : "hidden"
                    } p-1 bottom-10 w-32 rounded-md border border-gray-300 dark:border-border dark:bg-secondary shadow`}
                    onBlur={() => setDropdownOpen({})}
                  >
                    <ul
                      aria-labelledby="dropdownProjectButton"
                      className="divide-y divide-border text-sm text-gray-700 dark:text-gray-200 overflow-scroll max-h-32"
                    >
                      {projects.map(
                        (project) =>
                          project.alias !== "dalibook" && (
                            <li
                              key={project.alias}
                              className="cursor-pointer flex items-center py-1 first:pt-0 last:pb-0"
                              onClick={() => onDropdownItemClick(project.alias)}
                            >
                      ...
                      <li
                        key={"dalibook"}
                        className="cursor-pointer flex items-center py-1 first:pt-0 last:pb-0"
                        onClick={() => onDropdownItemClick("dalibook")}
                      >
                        <p className="cursor-pointer flex items-center rounded w-full px-2 py-1 gap-2 text-sm font-medium text-gray-400 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-tertiary">
                          <span className="p-1 rounded-full bg-gray-200 dark:bg-primaryhover text-primary">
                            <svg
                  ...
                  <button
                    className="px-2 py-1 text-primary rounded-full hover:bg-primaryhover text-sm font-medium flex flex-row gap-1 items-center"
                ...
                <div className="flex flex-row gap-2 items-center">
                  <p className="text-sm">
                    {MAX_CHARACTERS - textContent.length}
                  </p>
